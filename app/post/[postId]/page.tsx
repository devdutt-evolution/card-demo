...
export default function Post({ params }: { params: { postId: string } }) {
  ...
  const [post, setPost] = useState<PostComment>();
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const token = data?.user as any;
    setLoading(true);
    fetchPostDetail(params.postId, token?.token).then((post) => {
      setPost(post);
      setLoading(false);
    });
  }, [params.postId, data]);
        ...
        <>
          {post && Object.keys(post).length > 0 ? (
            <div className="bg-card rounded-lg py-6 px-8 m-2">
              <h2 className="text-2xl pb-5 font-bold">{post?.title}</h2>
              <div
                className="ProseMirror nono"
                dangerouslySetInnerHTML={{ __html: post.body }}
              ></div>
              <div className="border-2 border-divider my-4"></div>
              <Comment comments={post?.comments} token={data?.user} />
              <Link href="/posts">
